...
int fdSrc, fdDest; mode_t wrMode = 0777;
...
while ((bytes = read(fdSrc, &buf, 1)) >0) write(fdDest, &buf, 1);
if (bytes<0)
{
fprintf(stderr,"Could not read contents of %s\n", argv[1]);
return 2;
}
if (bytes == 0) write(fdDest, "\n", 1);
...
while ((bytes = read(fdSrc, &buf, 1)) > 0) printf("%c", buf);
close(fdDest);
return 0;
}
